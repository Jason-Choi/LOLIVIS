import { Selection, select, ScaleLinear, scaleLinear, interpolateBlues, interpolateReds, format, selectAll, DSVRowArray } from "d3";
import { Correlations, Attribute } from "../types";
...
function camelCaseToWords(str: string) {
    const strs =str.replace(/([A-Z])/g, " $1").replace(/^./, function(str) { return str.toUpperCase(); });
